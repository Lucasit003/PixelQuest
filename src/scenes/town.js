...
export class TownScene {
  ...
  _drawNight(g, Z) {
    ...
    if (!(W > 0 && H > 0)) return;
    const lights = this._nightLights(Z);

    // 1. the darkness
    const buf = this._nightBuffer(W, H);
    ...
    g.save();
    if (this._baseTf) g.setTransform(this._baseTf);
    ...
    this._drawNightMotes(g, Z);
    g.restore();
  }

  _nightBuffer(W, H) {
    let c = this._nightCanvas;
    if (!c) c = this._nightCanvas = document.createElement('canvas');
  ...
  _nightLights(Z) {
    ...
    const PZ = this.plazaCenter;
    if (PZ) add(PZ.x, PZ.y - 52, NIGHT.crystal, 0.90 + Math.sin(t * 1.25) * 0.10);
    for (const [x, y] of this.lamps) add(x, y - 19, NIGHT.lamp, 0.87 + Math.sin(t * 4 + x) * 0.13);
    for (const [x, y] of this.braziers) add(x, y - 10, NIGHT.fire, 0.78 + Math.sin(t * 8 + x) * 0.22);
    for (const [x, y, hue] of this.crystalGlows) {
  ...
  _drawNightMotes(g, Z) {
    ...
    const camX = Math.round(this.camX), camY = Math.round(this.camY);
    const W = this.W, H = this.H, t = this.t, S = NIGHT.moteGrid;
